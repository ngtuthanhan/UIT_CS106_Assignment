...
def myEvaluationFunction(currentGameState):
    ...
    if closestGhost:
        ghost_distance = -2 / closestGhost
    elif closestGhost > 10:
        ghost_distance = 0
    else:
        ghost_distance = -500
    
    list_eat_ghost = [manhattanDistance(newPos, ghost.getPosition()) - 1.5* ghost.scaredTimer for ghost in newGhostStates]
    
    if max(list_eat_ghost) > 0:
        can_eat_ghost = 0
    else:
        can_eat_ghost = sum(list_eat_ghost)

    foodList = newFood.asList()
    dist_foodList = [manhattanDistance(newPos, food) for food in foodList]
    dist_foodList.sort()
    if len(foodList) >=2:
        closestFood = dist_foodList[0]
        close2ndFood = dist_foodList[1]
    elif len(foodList) ==1:
        closestFood = dist_foodList[0]
        close2ndFood = 0 
    else:
    ...
    return  ghost_distance - 10 * len(foodList) - 2*closestFood - 0.897 * close2ndFood + score + 10* can_eat_ghost